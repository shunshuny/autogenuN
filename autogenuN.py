...
class AutoGenU(object):
    ...
    def set_functions(self, f, C, h, L, phi):
        ...
        lmd1 = sympy.symbols('lmd[0:%d]' %(self.__nx))
        lmd2 = sympy.symbols('lmd[0:%d]' %(self.__nx))
        lmd3 = sympy.symbols('lmd[0:%d]' %(self.__nx))
        lmd = Matrix([lmd1],[lmd2],[lmd3])
        hamiltonian1 = L[0] + sum(lmd1[i] * f[i] for i in range(self.__nx))
        hamiltonian2 = L[1] + sum(lmd2[i] * f[i] for i in range(self.__nx))
        hamiltonian3 = L[2] + sum(lmd3[i] * f[i] for i in range(self.__nx))
        #hamiltonian = Matrix([hamiltonian1,hamiltonian2,hamiltonian3])
        ...
        self.__symbolic_functions = SymbolicFunctions(f, phix1, phix2, phix3, hx1, hx2, hx3, hu1, hu2, hu3)
    ...
    def generate_ocp_definition(self, simplification: bool=False, common_subexpression_elimination: bool=False):
        ...
        if simplification:
            ...
            symutils.simplify(self.__hx1)
            symutils.simplify(self.__hx2)
            symutils.simplify(self.__hx3)
            symutils.simplify(self.__hu1)
            symutils.simplify(self.__hu2)
            symutils.simplify(self.__hu3)
            symutils.simplify(self.__phix1)
            symutils.simplify(self.__phix2)
            symutils.simplify(self.__phix3)
